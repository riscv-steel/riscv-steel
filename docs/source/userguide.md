...
This User Guide shows you how to develop a new embedded application with RISC-V Steel. The development process can be divided into two steps:

1. Writing the software for the application
2. Implementing and running the application on an FPGA

In the first step, you write the source code for the new application and build it using the RISC-V GNU Toolchain. The building process generates a `.hex` file that is used in the next step to initialize the memory of the Microcontroller IP.

In the second step, you create a new Verilog module with an instance of the Microcontroller IP and synthesize it for an FPGA. The Microcontroller IP instance is configured according to the FPGA and initialized with the application you wrote, which starts running as soon as you finish programming the FPGA.
...
- Then, using the FPGA vendor's EDA tool, you synthesize the wrapper module and program the FPGA to finally run the application.
